...
class FbpWindowsBleDevice implements BleDeviceWrapper {
  @override
  ...
  FbpWindowsBleDevice({required this.device});
...
class BleFbpWindowsRepository implements BleRepository {
  final _log = Logger("BleRepository");

  final Map<String, FbpWindowsBleDevice> _discoveredBleDevices = {};
  @override
  Map<String, BleDeviceWrapper> get discoveredBleDevices => _discoveredBleDevices;
  final _bleDeviceSubscription = StreamController<Map<String, FbpWindowsBleDevice>>.broadcast();
  ...
  Future<void> scan({List<String>? services, Duration? timeout = const Duration(seconds: 5)}) async {
    ...
    var scanSub = FlutterBluePlus.scanResults.listen((srs) async {
      for (var sr in srs) {
        ...
        _discoveredBleDevices[sr.device.remoteId.str] = FbpWindowsBleDevice(device: sr.device);
      }
