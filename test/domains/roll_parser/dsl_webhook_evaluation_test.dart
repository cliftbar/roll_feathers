...
Future<RuleParser> _parser(FakeDieDomain dd, FakeAppService app, http.Client client) async {
  final rd = await RollDomain.create(dd, app, httpClient: client);
  return rd.ruleParser;
}

void main() {
  // ── 3a — Async path ────────────────────────────────────────────────────────
  group('Evaluator async path — dispatch', () {
    ...
    test('3.1 POST fires when range matches', () async {
      ...
      await parser.runRuleAsync(_script(), [die]);
    ...
    test('3.6 payload matched_range has correct inclusive bounds for [5:15]', () async {
      final parser = await _parser(dd, app, rec.client);
      await parser.runRuleAsync(_script(range: '[5:15]'), [FakeDie('a', 'A', 10)]);
...
''';
      ...
      await parser.runRuleAsync(script, dice);
    ...
    test('3.9 all_dice contains all rolled dice', () async {
      ...
      await parser.runRuleAsync(_script(), dice);
    ...
    test('3.13 actions contains co-action blink with args', () async {
      final parser = await _parser(dd, app, rec.client);
      await parser.runRuleAsync(_scriptWithCoAction(), [FakeDie('a', 'A', 5)]);
...
''';
    ...
    test('3.16 GET fires with aggregate and rule as query params', () async {
      ...
      await parser.runRuleAsync(_script(method: 'GET', name: 'getRule'), [die]);
      expect(rec.requests.length, equals(1));
...
''';
  ...
  // ── 3b — Sync path ─────────────────────────────────────────────────────────
  group('Evaluator sync path — dispatch', () {
    late FakeDieDomain dd;
    late FakeAppService app;
    late _Recorder rec;
    ...
    test('3.19 sync path: webhook fires (fire-and-forget, awaiting pump)', () async {
      final parser = await _parser(dd, app, rec.client);
      final die = FakeDie('a', 'A', 5);
      // runRule is sync; the webhook future is fire-and-forget (.ignore())
      parser.runRule(_script(), [die]);
      // Pump event loop long enough for the ignored future to resolve
      await Future.delayed(const Duration(milliseconds: 100));
      expect(rec.requests.length, equals(1));
    });

    test('3.20 sync path: payload has only rule and aggregate (minimal payload)', () async {
      final parser = await _parser(dd, app, rec.client);
      parser.runRule(_script(name: 'syncRule'), [FakeDie('a', 'A', 4)]);
      await Future.delayed(const Duration(milliseconds: 100));
      final body = rec.bodies.first;
      expect(body.containsKey('rule'), isTrue);
      expect(body.containsKey('aggregate'), isTrue);
      // Full fields are absent in the sync path
      expect(body.containsKey('timestamp'), isFalse);
      expect(body.containsKey('result_dice'), isFalse);
      expect(body.containsKey('all_dice'), isFalse);
      expect(body.containsKey('actions'), isFalse);
    });
  });
}
