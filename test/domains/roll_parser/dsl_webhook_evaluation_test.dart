...
void main() {
  group('Evaluator — dispatch', () {
    ...
    test('3.1 POST fires when range matches', () async {
      ...
      await parser.runRule(_script(), [die]).runEffects();
    ...
    test('3.6 payload matched_range has correct inclusive bounds for [5:15]', () async {
      final parser = await _parser(dd, app, rec.client);
      await parser.runRule(_script(range: '[5:15]'), [FakeDie('a', 'A', 10)]).runEffects();
...
''';
      ...
      await parser.runRule(script, dice).runEffects();
    ...
    test('3.9 all_dice contains all rolled dice', () async {
      ...
      await parser.runRule(_script(), dice).runEffects();
    ...
    test('3.13 actions contains co-action blink with args', () async {
      final parser = await _parser(dd, app, rec.client);
      await parser.runRule(_scriptWithCoAction(), [FakeDie('a', 'A', 5)]).runEffects();
...
''';
      final parser = await _parser(dd, app, rec.client);
      await parser.runRule(script, [FakeDie('a', 'A', 3)]).runEffects();
      // Both hooks fire
...
''';
    ...
    test('3.16 GET fires with aggregate and rule as query params', () async {
      ...
      await parser.runRule(_script(method: 'GET', name: 'getRule'), [die]).runEffects();
      expect(rec.requests.length, equals(1));
