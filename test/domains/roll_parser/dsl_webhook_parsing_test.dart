...
void main() {
  group('Webhook DSL parsing', () {
    ...
    test('1.1 explicit POST parses method and URL', () {
      final t = _parseTarget('webhook POST https://example.com/roll');
      expect(t.rtType, equals(ResultTargetType.webhook));
      expect(t.action, equals('https://example.com/roll'));
      expect(t.args, equals(['POST']));
    });

    test('1.2 explicit GET parses method and URL', () {
      final t = _parseTarget('webhook GET https://example.com/roll');
      expect(t.rtType, equals(ResultTargetType.webhook));
      expect(t.action, equals('https://example.com/roll'));
    ...
    test('1.3 omitted method defaults to POST', () {
      final t = _parseTarget('webhook https://example.com/roll');
      expect(t.rtType, equals(ResultTargetType.webhook));
      expect(t.action, equals('https://example.com/roll'));
      expect(t.args, equals(['POST']));
    });
    ...
    test('1.4 method keyword is case-insensitive: lowercase get', () {
      final t = _parseTarget('webhook get https://example.com/hook');
      expect(t.args, equals(['GET']));
      expect(t.action, equals('https://example.com/hook'));
    });

    test('1.5 method keyword is case-insensitive: lowercase post', () {
      final t = _parseTarget('webhook post https://example.com/hook');
      expect(t.args, equals(['POST']));
      expect(t.action, equals('https://example.com/hook'));
    });

    test('1.6 URL with path segments preserved', () {
      final t = _parseTarget('webhook POST https://example.com/a/b/c');
      expect(t.action, equals('https://example.com/a/b/c'));
    });

    test('1.7 URL with query string preserved', () {
      final t = _parseTarget('webhook POST https://example.com/hook?foo=bar&baz=qux');
      expect(t.action, equals('https://example.com/hook?foo=bar&baz=qux'));
    });

    test('1.8 unknown method prefix treated as part of URL', () {
      // "DELETE" is not GET or POST, so treated as start of URL.
      final t = _parseTarget('webhook DELETE https://example.com');
      expect(t.args, equals(['POST']));
      expect(t.action, startsWith('DELETE'));
    });
...
''';
      ...
      expect(block.targets.first.targetFunction.action, equals('https://example.com/roll'));
    });
